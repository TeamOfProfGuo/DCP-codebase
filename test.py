...
def get_parser():
    ...
    parser = argparse.ArgumentParser(description='PyTorch Few-shot Segmentation')
...
def get_model(args):
    ...
    if args.weight:
        weight_path = osp.join(args.exp_dir, args.weight)
...
# ----------------------  VAL  ----------------------
    ...
    with open('./exp/test_record.log', 'a') as f:
...
def validate(val_loader, model, val_seed):
    ...
    for e in range(db_epoch):
        for i, (input, target, s_input, s_mask, cat_idx_list, ori_label) in enumerate(val_loader):
            ...
            with open('./exp/size_iou.log', 'a') as f:
