...
def get_model(args):
    ...
    # Resume
    get_save_path(args)
    check_makedirs(args.exp_dir)
    logger.info("=> exp_dir: {}".format(args.exp_dir))

    if args.resume:
        resume_path = osp.join(args.exp_dir, args.resume)
...
def main():
    ...
    train_data = dataset.SemData(split=args.split, shot=args.shot, data_root=args.data_root, data_list=args.train_list, \
                                    ...
                                    dataset=args.dataset, use_split_coco=args.use_split_coco)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch % args.save_freq == 0) and (epoch > 0) and main_process():
            filename = args.exp_dir + '/epoch_{}.pth'.format(epoch)
        ...
        # save model for <testing> and <fine-tuning>
            if mIoU > best_miou:
                ...
                keep_epoch = 0
                if args.shot == 1:
                    filename = args.exp_dir + '/train_epoch_' + str(epoch) + '_{:.4f}'.format(best_miou) + '.pth'
                else:
                    filename = args.exp_dir + '/train{}_epoch_'.format(args.shot) + str(epoch) + '_{:.4f}'.format(best_miou) + '.pth'
    ...
    for i, (input, target, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        # save model for <testing> and <fine-tuning>
            if mIoU > best_miou:
                ...
                keep_epoch = 0
                if args.shot == 1:
                    filename = args.exp_dir + '/train_epoch_' + str(epoch-0.5) + '_{:.4f}'.format(best_miou) + '.pth'
                else:
                    filename = args.exp_dir + '/train{}_epoch_'.format(args.shot) + str(epoch-0.5) + '_{:.4f}'.format(best_miou) + '.pth'
