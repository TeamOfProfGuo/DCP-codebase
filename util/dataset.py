...
class SemData(Dataset):
    def __init__(self, split=0, shot=1, data_root=None, data_list=None, dataset=None, use_split_coco=False, \
                        transform=None, mode='train', ann_type='mask', \
                        ft_transform=None, ft_aug_size=None, \
                        ms_transform=None):

        assert mode in ['train', 'val', 'demo', 'finetune']
        assert dataset in ['pascal', 'coco']
        if mode == 'finetune':
        ...
        fss_list_root = './lists/{}/fss_list/{}/'.format(dataset, self.mode)
